...
class PANet(nn.Module):
    def __init__(self):
        ...
        self.conv7 = nn.Sequential(
            CNNBlock(2048, 1024, kernel_size=3, stride=1, padding=1),
            CNNBlock(1024, 2048, kernel_size=1, stride=1, padding=0),
            CNNBlock(2048, 1024, kernel_size=3, stride=1, padding=1),
            CNNBlock(1024, 2048, kernel_size=1, stride=1, padding=0),
            CNNBlock(2048, 1024, kernel_size=3, stride=1, padding=1),
        )
    ...
    def forward(self, spp_out, route_connections):
        ...
        output_medium_part1 = self.conv2(x) # (1, 512, 26, 26)
        

        # print('out_big : ', output_big.shape)
        x = self.conv3(output_medium_part1) # (1, 256, 26, 26)
        x = self.upsample(x)       # (1, 256, 52, 52)
        x = torch.cat([x, route_connections[-2]], dim=1) # (1, 512, 52, 52)
        output_small = self.conv4(x)  # (1, 256, 52, 52)
        

        output_small_ds = self.conv5(output_small) # (1, 512, 26, 26)
        x = torch.cat([output_medium_part1, output_small_ds], dim=1) # (1, 1024, 26, 26)

        output_medium = self.conv2(x) # (1, 512, 26, 26)


        output_medium_ds = self.conv6(output_medium) # (1, 1024, 13, 13)
        x = torch.cat([spp_out, output_medium_ds], dim=1) # (1, 2048, 13, 13)
        output_big = self.conv7(x) # (1, 1024, 13, 13)
