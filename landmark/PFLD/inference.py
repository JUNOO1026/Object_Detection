...
from pfld_model import PFLDBackbone
from RetinaFace import RetinaFace
device = 'cuda' if torch.cuda.is_available() else 'cpu'

def landmark_detection(img, det, model_path):
    checkpoint = torch.load(model_path, map_loaction=device)
...
bounding_boxes, _ = RetinaFace(test_img_gray) # bounding boxes : [x1. y1. x2. y2, confidence]
